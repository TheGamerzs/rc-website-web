...
const ManagerCashoutTable = props => {
	...
	const toggleActive = () => {
		if (!showActive) {
			if (tables['RTS'])
				tables['RTS'].column(7).search('active').draw('full-hold');
			if (tables['PIGS'])
				tables['PIGS'].column(7).search('active').draw('full-hold');
			if (tables['BOTH'])
				tables['BOTH'].column(7).search('active').draw('full-hold');
		} else {
	...
	const Formatters = {
		rts: (cashout, i) => {
			...
			return (
				<tr key={i}>
					<th scope="row">
						{cashout.member.in_game_id} {cashout.member.in_game_name}
					</th>
					<td>
		...
		both: (cashout, i) => {
			...
			return (
				<tr key={i}>
					...
					<td>
						<FormattedNumber num={cashout.rts_cashout + cashout.pigs_cashout} />
					</td>
					<td>
						$
						<FormattedNumber
							num={cashout.rts_cashout_worth + cashout.pigs_cashout_worth}
						/>
					</td>
					<td>
						$
						<FormattedNumber
							num={cashout.rts_cashout * 10000 + cashout.pigs_cashout * 10000}
						/>
					</td>
					<td>
						$
	...
	function pay() {
		if (!payMember) return;
		setLoading(true);
		Api.payManager(payMember.id, payMember.company)
			.then(() => {
				props.refetch();
			})
			.catch(err => {
	...
	return (
		<React.Fragment>
			...
			/>
			{payMember && (
				<Modal isOpen={modal} toggle={toggle} fade scrollable>
					<ModalHeader toggle={toggle}>
						Pay {payMember.in_game_id} {payMember.in_game_name}
					</ModalHeader>
					<ModalBody>
						Are you sure you want to pay them for{' '}
						{payMember.company === 'both'
							? 'BOTH companys'
							: payMember.company.toUpperCase()}
						?
					</ModalBody>
					<ModalFooter>
