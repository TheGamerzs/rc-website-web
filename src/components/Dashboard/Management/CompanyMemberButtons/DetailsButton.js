...
const DetailsButton = props => {
	...
	function openDetails() {
		...
		if (!data && !loading && !error)
			getMemberPayouts({ variables: { member_id: member.id } });
	}

	return (
		<React.Fragment>
			<Button color="info" onClick={openDetails} disabled={modal}>
				Details
			</Button>
			<Modal isOpen={modal} toggle={toggle} fade size="xl">
				<ModalHeader toggle={toggle}>
					{member.in_game_name}'s Details
				</ModalHeader>
				<ModalBody>
					<b>Discord ID</b>: {member.discord_id}
					<br />
					<b>RTS Rank</b>: {member.rts_rank}
					<br />
					<b>PIGS Rank</b>: {member.pigs_rank}
					<br />
					{member.fire_reason && (
						<React.Fragment>
							<b>Fire Reason</b>: {member.fire_reason}
							<br />
						</React.Fragment>
					)}
					{member.company === 'fired' && (
						<Button
							disabled={loading}
							onClick={toggleRehire}
							color={member.welcome ? 'success' : 'danger'}>
							{loading ? (
								<LoadingIcon />
							) : member.welcome ? (
								'Currently Rehireable'
							) : (
								'Currently NOT Rehireable'
							)}
						</Button>
					)}
...
const formatter = (payout, key) => {
	return (
		<tr key={key}>
			<td>
				({payout.manager.member.in_game_id}){' '}
				{payout.manager.member.in_game_name}
			</td>
			<td>
